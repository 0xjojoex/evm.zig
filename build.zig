...
pub fn build(b: *std.Build) void {
    ...
    // ZisK and SP1 reserve fixed payload heaps after `_bss_end`. OpenVM uses
    // its platform allocator for both Zig and Rust, leaving one contiguous heap
    // for accelerators and post-run measurement.
    const guest_heap_bytes = b.option(
        u64,
        "guest-heap-bytes",
        "Fixed ZisK/SP1 guest payload heap capacity in bytes",
...
fn addGuest(
    ...
    if (unbuildable) |message| {
        ...
        if (config.run_step) |run_name| {
            const run_step = b.step(run_name, config.run_description.?);
...
fn buildNativePrecompileObject(
    ...
    module.addCSourceFile(.{ .file = b.path("src/precompile/kzg.c"), .flags = c_flags });
    return b.addObject(.{
...
// BLST must stay a static library: folding assembly.S into the relocatable
// native-precompiles object breaks the optimized Mach-O final link (internal
// assembly labels are discarded/rebased, so add_mod_384 falls through into
// add_mod_384x and corrupts the stack).
fn buildBlstLibrary(
    b: *std.Build,
    target: std.Build.ResolvedTarget,
    optimize: std.builtin.OptimizeMode,
    ...
    return b.addLibrary(.{
        .name = if (pic == true) "blst-pic" else "blst",
        .linkage = .static,
        .root_module = module,
    });
}
