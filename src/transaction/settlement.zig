...
pub const DefaultFees = struct {
    ...
    blob_count: usize = 0,
    blob_params: ?tx_blob.BlobParams = null,
};
...
pub fn Runtime(comptime spec: ExactSpec) type {
    return struct {
        ...
        pub fn defaultPlanFromGasPlan(_: Self, gas_limit: u64, plan: tx_gas.GasPlan, fees: DefaultFees) DefaultPlan {
            const upfront_debit = prechargeCost(transaction.blob_params, gas_limit, fees.gas_price, fees.blob_base_fee, fees.blob_count, fees.blob_params) orelse std.math.maxInt(u256);
            return .{
                .payer = fees.payer,
                .gas_limit = gas_limit,
...
fn prechargeCost(spec_params: ?tx_blob.BlobParams, gas_limit: u64, gas_price: u256, blob_base_fee: u256, blob_count: usize, blob_params: ?tx_blob.BlobParams) ?u256 {
    const gas_cost = checkedGasCost(gas_limit, gas_price) catch return null;
    const blob_gas = blobGasForCount(spec_params, blob_count, blob_params) orelse return null;
...
fn blobGasForCount(spec_params: ?tx_blob.BlobParams, blob_count: usize, blob_params: ?tx_blob.BlobParams) ?u256 {
    if (blob_count == 0) return 0;
    const params = blob_params orelse spec_params orelse return null;
    return tx_blob.blobGasForParams(params, blob_count);
}
