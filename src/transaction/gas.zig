...
pub const GasCharge = struct {
    ...
    pub fn total(self: GasCharge) error{Overflow}!u64 {
        return std.math.add(u64, self.regular, self.state);
    }
};
...
pub fn Runtime(comptime spec: ExactSpec) type {
    return struct {
        ...
        pub fn intrinsicGasForTransaction(
            _: Self,
            input: []const u8,
            options: IntrinsicGasOptions,
        ) error{Overflow}!u64 {
            var gas = try transaction.intrinsicBaseGas(options);
            if (options.is_create) {
                gas = try std.math.add(u64, gas, transaction.create_intrinsic_gas);
            }
            gas = try std.math.add(u64, gas, try transaction.calldataGas(input));
            if (options.access_list_counts.addresses != 0) {
                const count = std.math.cast(u64, options.access_list_counts.addresses) orelse
                    return error.Overflow;
                const cost = try std.math.mul(u64, count, transaction.access_list_address_gas);
                gas = try std.math.add(u64, gas, cost);
            }
            if (options.access_list_counts.storage_keys != 0) {
                const count = std.math.cast(u64, options.access_list_counts.storage_keys) orelse
                    return error.Overflow;
                const cost = try std.math.mul(u64, count, transaction.storage_key_gas);
                gas = try std.math.add(u64, gas, cost);
            }
            gas = try std.math.add(
                u64,
                gas,
                try transaction.accessListDataGas(options.access_list_counts),
            );
            if (options.is_create) {
                const initcode_word_charge = transaction.initcode_word_gas;
                if (initcode_word_charge != 0) {
                    const words = std.math.cast(u64, wordCount(input.len)) orelse
                        return error.Overflow;
                    const initcode_cost = try std.math.mul(u64, words, initcode_word_charge);
                    gas = try std.math.add(u64, gas, initcode_cost);
                }
            }
            if (options.authorization_count != 0) {
                const count = std.math.cast(u64, options.authorization_count) orelse
                    return error.Overflow;
                const cost = try std.math.mul(
                    u64,
                    count,
                    transaction.authorization_intrinsic_gas,
                );
                gas = try std.math.add(u64, gas, cost);
            }
            return gas;
        ...
        pub fn gasPlan(
            ...
            gas_limit: u64,
            options: IntrinsicGasOptions,
        ) error{Overflow}!GasPlan {
            const intrinsic_gas = try self.intrinsicGasForTransaction(input, options);
            const floor_gas = (try self.floorGasForTransaction(input, options)) orelse 0;
        ...
        pub fn minimumGasForTransaction(
            self: Self,
            input: []const u8,
            options: IntrinsicGasOptions,
        ) error{Overflow}!u64 {
            const intrinsic = try self.intrinsicGasForTransaction(input, options);
            const floor = (try self.floorGasForTransaction(input, options)) orelse return intrinsic;
            return @max(intrinsic, floor);
        }

        pub fn floorGas(self: Self, input: []const u8) error{Overflow}!?u64 {
            return self.floorGasForTransaction(input, .{});
        }

        pub fn floorGasForTransaction(
            _: Self,
            input: []const u8,
            options: IntrinsicGasOptions,
        ) error{Overflow}!?u64 {
...
test "transaction gas plan computes executable gas after intrinsic and floor costs" {
    ...
    const Prague = runtime(eth.prague);
    const Amsterdam = runtime(eth.amsterdam);
    const istanbul = try Istanbul.gasPlan(&.{ 0, 1 }, 100_000, .{});
    ...
    const prague_floor = try Prague.gasPlan(&.{ 1, 1, 1, 1 }, 21_100, .{});
    ...
    const amsterdam_floor = try Amsterdam.gasPlan(&.{ 1, 1, 1, 1 }, 16_000, .{});
    ...
    const amsterdam_access_list = try Amsterdam.gasPlan(&.{ 0, 1 }, 100_000, .{ .access_list_counts = .{
        .addresses = 1,
        .storage_keys = 3,
    } });
    ...
    const prague_authorization = try Prague.gasPlan(&.{}, 100_000, .{ .authorization_count = 1 });
...
test "transaction gas plan uses an extended exact spec" {
    const custom = struct {
        fn intrinsicBaseGas(_: IntrinsicGasOptions) error{Overflow}!u64 {
            return 5;
        }

        fn calldataGas(input: []const u8) error{Overflow}!u64 {
            var result: u64 = 0;
            for (input) |byte| {
                result = try std.math.add(u64, result, if (byte == 0) 2 else 3);
            }
            return result;
        }

        fn accessListDataGas(counts: AccessListCounts) error{Overflow}!u64 {
            return @as(u64, @intCast(counts.addresses + counts.storage_keys));
        }

        fn floorGas(_: FloorGasInput) error{Overflow}!?u64 {
            return 31;
        }
    };
    ...
    const plan = try CustomGas.gasPlan(&.{ 0, 1 }, 100, .{
...
test "transaction gas plan distinguishes absent floor from cost overflow" {
    const eth = @import("../eth.zig");
    const custom = struct {
        fn intrinsicOverflow(_: IntrinsicGasOptions) error{Overflow}!u64 {
            return error.Overflow;
        }

        fn floorOverflow(_: FloorGasInput) error{Overflow}!?u64 {
            return error.Overflow;
        }
    };

    const Frontier = runtime(eth.frontier);
    try std.testing.expectEqual(@as(?u64, null), try Frontier.floorGas(&.{}));
    const no_floor = try Frontier.gasPlan(&.{}, 21_000, .{});
    try std.testing.expectEqual(@as(u64, 0), no_floor.floor_gas);
    try std.testing.expect(no_floor.execution != null);

    const intrinsic_overflow_spec = eth.frontier.extend(.{ .transaction = .{
        .intrinsicBaseGas = custom.intrinsicOverflow,
    } });
    try std.testing.expectError(
        error.Overflow,
        runtime(intrinsic_overflow_spec).gasPlan(&.{}, std.math.maxInt(u64), .{}),
    );

    const floor_overflow_spec = eth.frontier.extend(.{ .transaction = .{
        .floorGas = custom.floorOverflow,
    } });
    try std.testing.expectError(
        error.Overflow,
        runtime(floor_overflow_spec).gasPlan(&.{}, std.math.maxInt(u64), .{}),
    );
}
...
test "Amsterdam gas plan executes only capped regular gas" {
    const eth = @import("../eth.zig");
    const Amsterdam = runtime(eth.amsterdam);
    const plan = try Amsterdam.gasPlan(&.{}, 120_000_000, .{});
