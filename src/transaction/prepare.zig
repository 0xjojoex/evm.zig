...
pub fn Runtime(comptime spec: ExactSpec) type {
    return struct {
        ...
        fn settlementPlanner(_: @This()) Settlement {
            return .{};
        }

        fn validationPlanner(_: @This()) Validation {
            return .{};
        }

        pub fn prepare(self: @This(), input: tx.PrepareInput) !tx.PrepareResult(settlement.DefaultPlan, validation.ValidationError) {
            const settlement_planner = self.settlementPlanner();
            const validation_planner = self.validationPlanner();
            const view = tx.transactionView(input.tx);
            ...
            const rejects_non_delegating = transaction.rejectsNonDelegatingSenderCode(view.kind);
            if (sender_account) |account| {
                if (rejects_non_delegating) {
                    validation_input.sender_code_kind = if (std.mem.eql(u8, &account.code_hash, &crypto.keccak256_empty))
                        ...
                        break :code_kind if (code.len == 0)
                            .empty
                        else if (transaction.isDelegationCode(code))
                            .delegation
                        else
                            .non_delegating;
                    };
            ...
            const settlement_plan = settlement_planner.defaultPlanFromGasPlan(view.gas_limit, gas_plan, .{
                .gas_price = gas_price,
                .priority_fee = settlement_planner.effectivePriorityFee(.{
                    .gas_price = gas_price,
                    .base_fee = input.env.base_fee,
                    .max_fee_per_gas = view.fee.max_fee_per_gas,
                    .max_priority_fee_per_gas = view.fee.max_priority_fee_per_gas,
