...
fn Easm(comptime spec: evmz.Spec) type {
    return struct {
        const instruction_table = instruction.InstructionTable(spec);
        ...
        pub const DisassembleIterator = struct {
            ...
            fn next(self: *DisassembleIterator) ?InstructionDetail {
                ...
                const instr = instruction_table.data[self.bytecode[self.pc]];
...
test "disassemble" {
    const easm = Easm(.cancun);

    var buf: [1024]u8 = undefined;
    const bytecode = try std.fmt.hexToBytes(&buf, "6311223344");

    var iter = easm.disassembleIter(bytecode);
    ...
    easm.disassemblePrint(bytecode1, .{});
}
