...
const Executor = @This();
const CallFramePool = std.heap.MemoryPool(Interpreter.CallFrame);
...
/// from the in-memory overlay. `trace_sink` is threaded through state and
/// interpreter frames when tracing is enabled.
...
pub fn executeTransactionMessage(self: *Executor, tx: Transaction, gas: u64) !Interpreter.Result {
    return switch (tx) {
        .call => |call_tx| self.executeCallTransaction(
            call_tx.sender,
            call_tx.recipient,
            call_tx.input,
            gas,
        ...
        .create => |create_tx| blk: {
            const result = (try self.executeCreateTransaction(
                create_tx.sender,
                create_tx.init_code,
                gas,
                create_tx.value,
...
pub fn runTopLevelTransactionWithEngine(
    ...
    const transaction_charged = if (run.execution_gas != null)
        try self.chargeTransactionCosts(sender, tx.gasLimit(), tx.value())
    else
        false;
    var authorization_refund: u64 = 0;
    if (transaction_charged) {
        if (!tx.isCreate()) {
            try self.incrementNonce(sender);
        }
        try self.warmAccessList(tx.accessList());
        authorization_refund = try self.applyAuthorizationList(tx.authorizationList());
    ...
    const authorization_refund_i64 = std.math.cast(i64, authorization_refund) orelse std.math.maxInt(i64);
...
pub fn applyAuthorizationList(self: *Executor, authorization_list: []const AuthorizationTuple) !u64 {
    if (!self.spec.isImpl(.prague)) return 0;
    var refund: u64 = 0;
    for (authorization_list) |auth| {
        refund = std.math.add(u64, refund, try self.applyAuthorizationTuple(auth)) catch std.math.maxInt(u64);
    }
    return refund;
}
...
///
/// Invalid tuples are ignored and return zero refund, matching transaction
/// execution rules.
pub fn applyAuthorizationTuple(self: *Executor, auth: AuthorizationTuple) !u64 {
    if (!self.spec.isImpl(.prague)) return 0;
    if (!eip7702.authorizationSignatureShapeValid(auth.y_parity, auth.legacy_v, auth.r, auth.s)) return 0;
    const tx_context = try call_runtime.currentTxContext(self);
    if (auth.chain_id != 0 and auth.chain_id != tx_context.chain_id) return 0;
    if (auth.nonce == std.math.maxInt(u64)) return 0;
    ...
    if (existing_account) |existing| {
        if (existing.code.len != 0 and eip7702.delegationTarget(existing.code) == null) return 0;
        if (existing.nonce != auth.nonce) return 0;
    ...
    return if (account_exists) transaction.authorization_existing_account_refund_gas else 0;
}
...
test "executor executeTransactionMessage dispatches normalized call" {
    ...
    try executor.beginTransactionScope(tx_context, tx);
    const result = try executor.executeTransactionMessage(tx, 100_000);
...
test "executor returns EIP-7702 refund for existing authority account" {
    ...
    try std.testing.expectEqual(transaction.authorization_existing_account_refund_gas, refund);
...
test "executor rejects EIP-7702 max authorization nonce before warming signer" {
    ...
    try std.testing.expectEqual(@as(u64, 0), refund);
...
test "recursive call bomb unwinds with default executor" {
    ...
    try executor.beginTransaction(tx_context, sender, contract);
    const result = try executor.executeCallTransaction(sender, contract, &.{}, 979_000, 100_000);
...
test "top-level call transaction executes precompile recipient" {
    ...
    const result = try executor.executeCallTransaction(sender, precompile, &input, 1000, 7);
...
test "create address collision closes checkpoint without rolling back nonce or warmth" {
    ...
    const result = (try executor.executeCreateTransaction(sender, &.{0x00}, 100_000, 1)).expectCreate();
