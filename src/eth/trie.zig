//! Ethereum execution MPT helpers for root construction, proof lookup, the
//! witness catalog, and hashed-key updates. Post-state roots from execution
//! changes live in `eth.commit`.
...
/// Hashed-key seam for `eth.commit`. Keeping the structural trie context here
/// avoids instantiating a second Keccak-backed MPT implementation merely
/// because commit lives in its own source file.
