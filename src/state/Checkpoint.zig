...
//! of which state model is compiled in, so this shape belongs to neither the
//! tracked nor the claim-indexed lane.
//!
//! Every lane fills every field; a lane that has no counterpart for a field
//! fills the value that makes its close a no-op.
