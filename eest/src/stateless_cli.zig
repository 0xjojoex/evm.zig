...
const Worker = struct {
    ...
    fn run(self: *Worker, io: std.Io, queue: *std.Io.Queue([]u8)) std.Io.Cancelable!void {
        while (true) {
            ...
            const summary = stateless.runFile(io, self.allocator, path, self.options) catch |err| {
...
fn requiresSequential(options: stateless.Options) bool {
    return options.limit > 0 or options.verbose or options.trace_mismatch or options.classify_failures;
}
...
fn printUsage() void {
    std.debug.print(
        \\usage: zig build zkevm -- [--jobs N] [--test NAME] [--limit N] [--verbose] [--trace-mismatch] [--classify-failures] [--ere-public] [path ...]
        \\
...
test "jobs parser enforces the zkEVM memory bound" {
